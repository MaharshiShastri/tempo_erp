...
export default function DispatchPlannerView({ state }) {
    ...
    const [dim, setDim] = useState({ width: 0, height: 0, depth: 0, weight: 0, invoice_value: 0, destination_city: "", diesel_price: 98, cft_charge: 0, min_weight: 0, hamali_detail: "", hamali_cost: 0
    });
    ...
    const handleEvaluate = async (e) => {
        e.preventDefault();
        try {
            const payload = {...dim, partner_distances: partnerDistances};
            ...
            <div className="system-header">
              ...
              <form onSubmit={handleEvaluate} style={{ background: "var(--bg-main)", padding: "20px", borderRadius: "var(--radius-sm)", marginBottom: "20px", border: "1px solid var(--border-light)" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: "15px", marginTop: "10px" }}>
                    {partners.map((partner) => ( <div key={partner.id} style={{ border: "1px solid var(--border-light)", borderRadius: "8px", padding: "15px", background: "var(--bg-surface)" }}>
                        <h4 style={{ margin: 0 }}>
                            {partner.name}
                        </h4>

                        <p style={{ marginTop: "4px", fontSize: "12px", color: "var(--text-muted)"}}>
                            {partner.partner_link && (<a href={partner.partner_link} style={{color: 'var(--text-primary)'}} target="_blank" rel="noopener noreferrer" title="Open Partner Website">Go to {partner.name} website to find the distance.<FiExternalLink size={18} /></a>)}
                        </p>

                        <label className="input-label">
                            Distance from Local Hub (KM)
                        </label>
